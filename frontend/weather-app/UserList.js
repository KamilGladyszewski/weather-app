     ...
     fetch (baseURL + query + "&format=json&lang=pl-PL")
      ...
      .then(json => {
        if(json.query.results){
          var isOk = true;
          if(json.query.results.channel.location.country.toLowerCase() !== data.get("country").toLowerCase() ||
          json.query.results.channel.location.city.toLowerCase() !== data.get("city")){
            if(window.confirm("Did you mean: " + data.get("city") + ", " + json.query.results.channel.location.country + "?")){
              data.set("country", json.query.results.channel.location.country)
              data.set("city", json.query.results.channel.location.city)
            } else {
              isOk = false;
            }
          }
          if(isOk){
            ...
            .then(json2 => {
              ...
              this.setState({
                fetchedCountry: json.query.results.channel.location.country
              })
            })
          }else {
              this.setState ({
          ...
          return (
            <tr className="user-tr" onClick = {this.addingStart}>
              <td colSpan="6" style = {{textAlign: "center"}}>
                Forecast not found for location: {this.state.city}, {this.state.dataCountry}
                <i className="fas fa-plus-circle col"></i>
              </td>
            </tr>
          )
  ...
  render() {
    if(!this.state.deleted){
      if(!this.state.editing){
        ...
        return (
          <tr className="user-tr">
                <td><input type="text" name="fname" defaultValue={this.props.user.fname} placeholder={this.state.user.fname} form={"userForm" + this.props.user.id}/></td>
                <td><input type="text" name="lname"defaultValue={this.props.user.lname} form={"userForm" + this.state.user.id}/></td>
                <td><input type="text" name="city" defaultValue={this.props.user.city} form={"userForm" + this.state.user.id}/></td>
                <td><input type="text" name="country" defaultValue={this.props.user.country} form={"userForm" + this.state.user.id}/></td>
                <td>
